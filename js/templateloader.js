...
function loadTemplate(element, templateFile) {
    ...
    fetch(templateFile) // Adjust the path as needed
        ...
        .then(template => {
            ...
            setupHoverEffects(); // Call this after loading the template to ensure hover effects work on new elements
        })
...
function loadMarkdown(element, markdownFile, encode=true) {
    const container = document.getElementById(element);
    if (encode) {
    ...
    fetch(CONTENT_FOLDER + markdownFile)
        ...
        .then(markdown => {
            ...
            // Render math equations, then apply scratch
...
function makeCustomRenderer() {
    ...
    renderer.del = function (text) {
        return `<del class="scratch">${text}</del>`;
    };

    renderer.image = function (href, title, text) {
        const audioExtensions = /\.(mp3|opus|ogg|wav|m4a)$/i;
        // Audio into a soundplayer
        if (audioExtensions.test(href)) {
            return `
                <audio controls preload="none" class="audio-player w-full" title="${title || ''}">
                    <source src="${href}">
